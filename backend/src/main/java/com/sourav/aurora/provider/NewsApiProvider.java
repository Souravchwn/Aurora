...
public class NewsApiProvider extends AbstractNewsProvider {
    ...
    protected String buildUrl(String country, String language, String category, String keyword, int page, int pageSize) {
        ...
        boolean hasKeyword = keyword != null && keyword.trim().length() >= ApplicationConstants.MIN_KEYWORD_LENGTH;

        // /everything supports: q, language, sortBy, pageSize, page
        // /top-headlines supports: country, category, q, pageSize, page  (language is NOT valid here)
        // Route: keyword present → /everything;  country/category browsing → /top-headlines
        if (hasKeyword) {
        ...
        // /top-headlines — no language param (not valid for this endpoint)
        ...
        if (isSupported(category, ApplicationConstants.SUPPORTED_CATEGORIES)) {
            builder.queryParam("category", mapCategory(category));
        }

        // /top-headlines requires at least one of: country, sources, q
        // If neither country nor category was added, add a default q to avoid 400
        boolean hasCountry = isSupported(country, ApplicationConstants.SUPPORTED_COUNTRIES);
        if (!hasCountry) {
            builder.queryParam("q", "world");
        }
